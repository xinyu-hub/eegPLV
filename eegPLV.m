...
%   [plv] = eegPLV(eegData, srate, filtSpec, dataSelectArr);
...
d = designfilt("bandpassfir", ...
    FilterOrder=filtSpec.order,CutoffFrequency1=filtSpec.range(1), ...
    CutoffFrequency2=filtSpec.range(2),SampleRate=srate);

for i=1:size(eegData,1)
    parfor j=1:size(eegData,3)
        filteredData(i,:,j)=filtfilt(d,eegData(i,:,j));
    end
end
